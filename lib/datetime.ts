...
/**
 * Formatta una data (con ora) per la VISUALIZZAZIONE, forzando sempre il
 * fuso orario di Roma — indipendentemente da dove gira il codice (il
 * server su Vercel è in UTC). Senza `timeZone: "Europe/Rome"`,
 * `toLocaleString` userebbe il fuso orario di ESECUZIONE, mostrando
 * l'orario UTC grezzo invece di quello reale di Roma (stesso problema di
 * `parseDatetimeLocalRoma`, ma lato lettura invece che scrittura).
 */
export function formatDataOraRoma(date: Date | string, opzioni?: Intl.DateTimeFormatOptions): string {
...
/** Come formatDataOraRoma ma senza l'orario, solo il giorno. */
export function formatSoloDataRoma(date: Date | string, dateStyle: "medium" | "long" = "medium"): string {
  return new Date(date).toLocaleDateString("it-IT", { dateStyle, timeZone: FUSO_ORARIO });
}
