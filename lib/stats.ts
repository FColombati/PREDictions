...
/**
 * Striscia di sconfitte consecutive (corrente e migliore) — speculare a
 * calcolaStreak ma per le sconfitte. Se `tournamentId` è indicato,
 * considera solo le schedine di quel torneo.
 */
export async function calcolaStreakSconfitte(userId: string, tournamentId?: string) {
  const scores = await prisma.userScore.findMany({
    where: { userId, ...(tournamentId ? { match: { tournamentId } } : {}) },
    include: { match: { select: { data: true } } },
    orderBy: { match: { data: "asc" } },
  ...
  for (const s of scores) {
    ...
    const persa = valori.filter((d) => d.corretta).length / valori.length <= 0.5;

    if (persa) {
      inCorso += 1;
      peggiore = Math.max(peggiore, inCorso);
    } else {
      inCorso = 0;
    }
  }

  return { corrente: inCorso, peggiore };
}

/**
 * Il punteggio più alto mai ottenuto in una singola schedina (partita o
 * torneo). Se `tournamentId` è indicato, considera solo le schedine di
 * quel torneo.
 */
export async function punteggioMassimoSchedina(userId: string, tournamentId?: string) {
  const [scoresPartita, scoresTorneo] = await Promise.all([
    prisma.userScore.findMany({ where: { userId, ...(tournamentId ? { match: { tournamentId } } : {}) } }),
    prisma.tournamentScore.findMany({ where: { userId, ...(tournamentId ? { tournamentId } : {}) } }),
  ]);
  const tutti = [...scoresPartita, ...scoresTorneo];
  if (tutti.length === 0) return 0;
  return Math.max(...tutti.map((s) => s.punti));
}

/**
 * Conta le schedine in cui l'utente ha sbagliato TUTTE le domande
 * "contabili" (0% corrette, almeno una domanda). Se `tournamentId` è
 * indicato, considera solo le schedine di quel torneo.
 */
export async function contaSchedineFalliteTotalmente(userId: string, tournamentId?: string) {
  const [scoresPartita, scoresTorneo] = await Promise.all([
    prisma.userScore.findMany({ where: { userId, ...(tournamentId ? { match: { tournamentId } } : {}) } }),
    prisma.tournamentScore.findMany({ where: { userId, ...(tournamentId ? { tournamentId } : {}) } }),
  ]);
  ...
  for (const s of [...scoresPartita, ...scoresTorneo]) {
    const valori = valoriContabili(s.dettaglio);
    if (valori.length > 0 && valori.every((d) => !d.corretta)) fallite += 1;
  }
  return fallite;
}
