...
import {
  ...
  punteggioMassimoSchedina,
  contaSchedineFalliteTotalmente,
...
export type TriggerType =
  ...
  | "STREAK_UPDATED"
  | "LOSE_STREAK_UPDATED"
  | "PUNTEGGIO_SCHEDINA_ALTO"
  | "SCHEDINA_FALLITA_TOTALE"
  | "PUNTI_TOTALI_CARRIERA"
  | "ACCOUNT_CREATED"
  | "MANUAL";
...
async function calcolaValoreMetrica(
  ...
  switch (trigger) {
    ...
    case "LOSE_STREAK_UPDATED": {
      const streak = await calcolaStreakSconfitte(userId, tournamentId ?? undefined);
      return streak.corrente;
    }
    case "PUNTEGGIO_SCHEDINA_ALTO":
      return punteggioMassimoSchedina(userId, tournamentId ?? undefined);
    case "SCHEDINA_FALLITA_TOTALE":
      return contaSchedineFalliteTotalmente(userId, tournamentId ?? undefined);
    case "PUNTI_TOTALI_CARRIERA": {
      if (tournamentId) {
        const m = await metrichePronosticiTorneo(userId, tournamentId);
        return m.puntiTotali;
      }
      const stats = await statisticheProfilo(userId);
...
async function sbloccaSeNuovo(
  ...
  await creaNotifica(userId, "ACHIEVEMENT_UNLOCKED", `Hai sbloccato l'achievement "${achievement.nome}"!`, {
