...
import { eseguiInGruppi } from "@/lib/concurrency";
...
export async function aggiornaStatoPartita(matchId: string, stato: string) {
  ...
  if (stato === "ANNULLATA") {
    ...
    await eseguiInGruppi(scoresAnnullata, 10, async (s) => {
      await Promise.all([
        valutaTrigger(s.userId, "PREDICTION_WON", { tournamentId: match.tournamentId }),
        valutaTrigger(s.userId, "PREDICTION_LOST", { tournamentId: match.tournamentId }),
        valutaTrigger(s.userId, "ACCURACY_UPDATED", { tournamentId: match.tournamentId }),
        valutaTrigger(s.userId, "STREAK_UPDATED", { tournamentId: match.tournamentId }),
        valutaTrigger(s.userId, "LOSE_STREAK_UPDATED", { tournamentId: match.tournamentId }),
        valutaTrigger(s.userId, "PUNTEGGIO_SCHEDINA_ALTO", { tournamentId: match.tournamentId }),
        valutaTrigger(s.userId, "SCHEDINA_FALLITA_TOTALE", { tournamentId: match.tournamentId }),
        valutaTrigger(s.userId, "PUNTI_TOTALI_CARRIERA", { tournamentId: match.tournamentId }),
      ]);
    });
...
export async function calcolaPunteggi(matchId: string) {
  ...
  if (match) {
    await eseguiInGruppi(match.scores, 10, async (s) => {
      await Promise.all([
        valutaTrigger(s.userId, "PREDICTION_WON", { tournamentId: match.tournamentId }),
        valutaTrigger(s.userId, "PREDICTION_LOST", { tournamentId: match.tournamentId }),
        valutaTrigger(s.userId, "ACCURACY_UPDATED", { tournamentId: match.tournamentId }),
...
export async function calcolaPunteggiTorneoAction(tournamentId: string) {
  ...
  await eseguiInGruppi(scores, 10, async (s) => {
    await Promise.all([
      valutaTrigger(s.userId, "PREDICTION_WON", { tournamentId }),
      valutaTrigger(s.userId, "PREDICTION_LOST", { tournamentId }),
      valutaTrigger(s.userId, "ACCURACY_UPDATED", { tournamentId }),
      valutaTrigger(s.userId, "PUNTEGGIO_SCHEDINA_ALTO", { tournamentId }),
      valutaTrigger(s.userId, "SCHEDINA_FALLITA_TOTALE", { tournamentId }),
      valutaTrigger(s.userId, "PUNTI_TOTALI_CARRIERA", { tournamentId }),
      valutaTrigger(s.userId, "TOURNAMENT_WON", { tournamentId }),
      valutaTrigger(s.userId, "TOURNAMENT_FINISHED", { tournamentId }),
    ]);
  });
