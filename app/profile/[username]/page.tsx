...
export default async function ProfiloPubblicoPage({
  ...
  const [stats, userAchievements, badgeTooltips] = await Promise.all([
    statisticheProfilo(user.id),
    prisma.userAchievement.findMany({
      where: { userId: user.id, sbloccato: true },
      include: { achievement: true },
      orderBy: { sbloccatoIl: "asc" },
    }),
    Promise.all(
  ...
  const gallery = userAchievements
    .filter((ua) => !ua.achievement.eliminato)
    .map((ua) => {
    ...
    <div className={`mx-auto max-w-4xl px-4 py-10 sm:px-6 ${classeTema}`}>
      ...
      {user.equippedBackground && (
        <CosmeticStyleTag
          ...
          defaultAsset="background-repeat: no-repeat; background-size: 100% 100%; background-position: center; background-color: var(--color-panel-2);"
        />
      )}
      ...
      {gallery.length === 0 ? (
        <p className="panel-cut p-6 text-sm text-text-muted">Nessun achievement sbloccato ancora.</p>
      ) : (
